...
- 시드를 새로 받으려면 `docker compose down -v` 로 DB 볼륨을 지운 뒤 다시 띄우면 됩니다.
...
라벨은 `[필수]` 명세 핵심 / `[선택]` 명세 선택(추가 점수) / `[추가]` 자발적 차별화로 구분합니다. 상세 분류는 [docs/SCOPE.md](docs/SCOPE.md) 에 있습니다.

- **`[필수]` 10/10** — 강의 등록/상태전이(DRAFT→OPEN→CLOSED)/목록(status 필터)/상세(신청 인원 포함), 수강 신청/결제 확정/취소/내 신청 목록, 정원 초과 거부, 동시성 제어.
- **`[선택]` 4/4** — 결제 후 7일 이내 취소 제한 · 대기열(등록·크리에이터 조회) · 강의별 수강생 목록(크리에이터 전용) · 신청 내역 페이지네이션 (강의 목록 페이지네이션도 추가했습니다).
- **`[추가]` (차별화)** — 결제 멱등성(Idempotency-Key) · 명시적 FSM · 대기열 자동 승급(`FOR UPDATE SKIP LOCKED`) · 동시성 다층 방어(비관 락 + `@Version` + 부분 UNIQUE 인덱스) · Testcontainers 동시성 통합 테스트 · K6 부하 테스트 · OpenAPI/Swagger · RFC 7807 ProblemDetail 에러 응답 · 구조화 로깅(JSON + traceId) · Docker Compose · GitHub Actions CI · Actuator · Mermaid ERD/시퀀스 다이어그램.
...
4-Layer 방어로 구성합니다. ① row-level 비관 락(직렬화) ② `@Version` 낙관 락(stale write 차단 → `OPTIMISTIC_LOCK_CONFLICT`) ③ 부분 UNIQUE 인덱스(동일 강의 중복 신청 → `DATA_INTEGRITY_VIOLATION`) ④ `Idempotency-Key` UNIQUE(결제 경합). 취소로 자리가 비면 대기열의 다음 사람을 `FOR UPDATE SKIP LOCKED` 로 안전하게 한 명 잡아 PENDING 으로 자동 승급합니다.

> 이전에 수행한 유사 과제(Python/FastAPI)는 `threading.Lock`(단일 프로세스 한정)에 의존했습니다. 이번에는 DB 기반이라 다중 인스턴스에서도 정합이 보장됩니다.

상세는 [docs/CONCURRENCY.md](docs/CONCURRENCY.md) 에 있습니다. 검증은 `./gradlew test --tests ConcurrencyTest`(Testcontainers)와 `k6 run load-test/enrollment-burst.k6.js`(앱 실행 후)로 합니다.
...
- **DB로 PostgreSQL** (H2/MySQL 대신) — "동일 강의에 active 신청 1개, 취소 후 재신청 허용"을 **부분 UNIQUE 인덱스**(`WHERE status <> 'CANCELLED'`)로, 대기열 자동 승급을 `FOR UPDATE SKIP LOCKED` 로 DB 레벨에서 그대로 표현하기 위해서입니다. H2 는 부분 인덱스를 지원하지 않고 MySQL 은 우회가 필요합니다. 즉시 실행성은 Docker Compose 로, 통합 테스트는 Testcontainers 로 보완합니다.
- **정원 동시성에 비관 락** (`SELECT … FOR UPDATE` on `Lecture` row) — 정원은 경합이 몰리는 핫스팟이라 낙관 락 단독이면 재시도 루프가 필요하고 tail latency 가 나빠집니다. 비관 락은 차례로 직렬화해 재시도 없이 정확하고, "락 잡은 뒤 검사 = 커밋 시점"이라 로직이 단순하며, 단일 row 락이라 데드락이 없습니다. `@Version` 은 락 밖 경로(마이그레이션·버그)의 stale write 를 잡는 보조 방어입니다(→ `OPTIMISTIC_LOCK_CONFLICT`). 트레이드오프로 같은 강의 신청이 직렬화되지만, 락 단위가 강의(row)라 시스템 전체는 강의 수만큼 병렬입니다.
- **결제 확정에 Idempotency-Key 헤더 + DB UNIQUE** — 재시도·더블 클릭에도 한 번만 처리합니다(같은 키 재호출 시 동일 응답, 동시 경합은 `payment_intents.idempotency_key` UNIQUE 가 최종 방어). 결제 API 의 표준 패턴입니다.
- **상태 전이를 도메인 메서드에** (`Lecture#changeStatus`, `Enrollment#confirm/cancel`) — 잘못된 전이를 도메인에서 차단하고, 서비스가 `BusinessException`(+ `ErrorCode`)으로 래핑하면 `GlobalExceptionHandler` 가 RFC 7807 ProblemDetail 로 변환합니다.
...
- **인증/인가**: `X-User-Id` 헤더만 사용합니다(명세 허용). JWT/세션·비밀번호·리프레시 토큰은 미구현이며, 프로덕션 전환 시 Spring Security + JWT 로 교체하면 됩니다.
- **결제 PG 연동**: 외부 결제 시스템 연동 없이 상태 변경으로 대체했습니다(명세 허용). `PaymentIntent` 의 PENDING/FAILED 라이프사이클은 스키마에만 존재하며 현재 흐름은 COMPLETED 만 사용합니다.
- **분산 락**: 단일 인스턴스 + DB row 락으로 정합을 보장합니다. 다중 인스턴스로 스케일 아웃 시 Redis(Redisson) 등을 검토할 수 있으나 본 과제 범위 밖입니다.
- **PENDING 자동 만료**: 무한 PENDING 점유 방지 정책(예: 24h TTL 후 자동 취소)은 미구현이며 운영 정책이 미정입니다(REQUIREMENTS BR-7 참조).
- **알림 발송**: BE-A 범위 밖입니다.
- **컨트롤러 MockMvc 테스트**: 컨트롤러가 얇은 pass-through 라 서비스/도메인 단위 + 통합 테스트로 핵심을 커버했습니다. 여력이 있으면 `@WebMvcTest` 로 에러 응답 포맷 검증을 추가할 수 있습니다.
