# be-enrollment-assignment — 수강 신청 시스템 (BE-A)

[![CI](https://github.com/dolmaroyujinpark/be-enrollment-assignment/actions/workflows/ci.yml/badge.svg)](https://github.com/dolmaroyujinpark/be-enrollment-assignment/actions/workflows/ci.yml)
...
| Repository | <https://github.com/dolmaroyujinpark/be-enrollment-assignment> (`main` 브랜치 실행 가능) |
...
코드·문서·테스트 초안 작성에 AI 도구(Claude)를 사용했으며, 산출물은 작은 단위로 검증·수정한 뒤 의미 단위로 직접 커밋했습니다.

검증 사이클을 세 층으로 운영했습니다:
- **race 카테고리 매트릭스 직접 점검** — 3건 발견 후 회귀 테스트로 고정 (동시 cancel · 결제 멱등 리플레이 시 타 사용자 정보 노출 · CLOSED 강의 잘못된 자동 승급)
- **Codex 로 교차 검증** — 주요 설계 결정과 문서 정합성을 다른 AI 로 한 번 더 점검해 정확도를 높임 (예: 비관 락이 모든 Lecture 변경을 직렬화하므로 `Lecture.@Version` 이 불필요하다는 결론에 도달)
- **평가자 모드 수동 테스트** — Swagger UI 로 직접 점검해 단위 테스트가 못 잡던 sort placeholder 500 버그 발견 → 400 매핑

상세 활용 내역과 검증 절차는 [`docs/AI_USAGE.md`](docs/AI_USAGE.md) 에 정리되어 있습니다.

## 디렉토리 구조

```
be-enrollment-assignment/
├── README.md
