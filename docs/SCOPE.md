...
명세가 명시한 것(필수 / 선택)과, 명세에 직접 적히지 않았지만 "제대로 동작하려면" 필요한 것, 자발적으로 더한 것을 구분해 정리합니다. 모든 항목에 코드 위치를 적어 1:1 추적이 가능하게 했습니다.
...
## 3. 명세에 없지만 "제대로 하려면" 필요한 것

명세에 한 줄로만 적혔거나 직접 적히지 않았지만, 시스템이 실제로 올바르게 동작하려면 필요한 것들입니다.
